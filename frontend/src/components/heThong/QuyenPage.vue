...
import { createQuyen, updateQuyen, deleteQuyen } from '@/api/heThong/ApiRequest/QuyenController.js';

const pageTitle = 'Quyền';
const fields = [
  {
    "key": "maQuyen",
    "label": "Ma quyen",
...
const submitForm = async () => {
  ...
  try {
    ...
    if (editingId.value) {
      await updateQuyen(editingId.value, payload);
    } else {
      await createQuyen(payload);
    }
    resetForm();
    await loadData();
  } catch (err) {
