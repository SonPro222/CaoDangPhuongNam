...
import { createVaiTroQuyen, updateVaiTroQuyen, deleteVaiTroQuyen } from '@/api/heThong/ApiRequest/VaiTroQuyenController.js';

const pageTitle = 'Vai trò quyền';
const fields = [
  {
    "key": "vaiTroId",
    "label": "Vai tro id",
...
const loadData = async () => {
  ...
  error.value = '';
  try {
    const response = await getAllVaiTroQuyen();
...
const submitForm = async () => {
  ...
  try {
    ...
    if (editingId.value) {
      await updateVaiTroQuyen(editingId.value, payload);
    } else {
      await createVaiTroQuyen(payload);
    }
    resetForm();
    await loadData();
  } catch (err) {
