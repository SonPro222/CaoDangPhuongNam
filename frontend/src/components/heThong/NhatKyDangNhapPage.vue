...
import { getAllNhatKyDangNhap } from '@/api/heThong/ApiRespone/NhatKyDangNhapController.js';
import { createNhatKyDangNhap, updateNhatKyDangNhap, deleteNhatKyDangNhap } from '@/api/heThong/ApiRequest/NhatKyDangNhapController.js';

const pageTitle = 'Nhật ký đăng nhập';
const fields = [
  {
    "key": "taiKhoanId",
    "label": "Tai khoan id",
...
const loadData = async () => {
  ...
  error.value = '';
  try {
    const response = await getAllNhatKyDangNhap();
...
const submitForm = async () => {
  ...
  try {
    ...
    if (editingId.value) {
      await updateNhatKyDangNhap(editingId.value, payload);
    } else {
      await createNhatKyDangNhap(payload);
    }
    resetForm();
    await loadData();
  } catch (err) {
