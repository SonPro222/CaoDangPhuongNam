...
import { createTaiKhoanVaiTro, updateTaiKhoanVaiTro, deleteTaiKhoanVaiTro } from '@/api/heThong/ApiRequest/TaiKhoanVaiTroController.js';

const pageTitle = 'Tài khoản vai trò';
const fields = [
  {
    "key": "taiKhoanId",
    "label": "Tai khoan id",
...
const loadData = async () => {
  ...
  error.value = '';
  try {
    const response = await getAllTaiKhoanVaiTro();
...
const submitForm = async () => {
  ...
  try {
    ...
    if (editingId.value) {
      await updateTaiKhoanVaiTro(editingId.value, payload);
    } else {
      await createTaiKhoanVaiTro(payload);
    }
    resetForm();
    await loadData();
  } catch (err) {
