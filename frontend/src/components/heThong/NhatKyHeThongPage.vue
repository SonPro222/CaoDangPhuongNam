...
import { getAllNhatKyHeThong } from '@/api/heThong/ApiRespone/NhatKyHeThongController.js';
import { createNhatKyHeThong, updateNhatKyHeThong, deleteNhatKyHeThong } from '@/api/heThong/ApiRequest/NhatKyHeThongController.js';

const pageTitle = 'Nhật ký hệ thống';
const fields = [
  {
    "key": "taiKhoanId",
    "label": "Tai khoan id",
...
const loadData = async () => {
  ...
  error.value = '';
  try {
    const response = await getAllNhatKyHeThong();
...
const submitForm = async () => {
  ...
  try {
    ...
    if (editingId.value) {
      await updateNhatKyHeThong(editingId.value, payload);
    } else {
      await createNhatKyHeThong(payload);
    }
    resetForm();
    await loadData();
  } catch (err) {
