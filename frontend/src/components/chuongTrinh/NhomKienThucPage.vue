...
const filteredItems = computed(() => {
  ...
  return items.value.filter((item) => String(item.ma || '').toLowerCase().includes(keyword) || String(item.ten || '').toLowerCase().includes(keyword) || String(item.loaiNhom || '').toLowerCase().includes(keyword));
});
