...
const filteredItems = computed(() => {
  ...
  return items.value.filter((item) => String(item.ten || '').toLowerCase().includes(keyword) || String(item.tacGia || '').toLowerCase().includes(keyword) || String(item.nhaXuatBan || '').toLowerCase().includes(keyword) || String(item.loai || '').toLowerCase().includes(keyword));
});
